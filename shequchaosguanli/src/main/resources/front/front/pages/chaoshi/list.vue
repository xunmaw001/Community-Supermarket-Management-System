...
<template>
	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
		...
		<view class="search-form round">
			...
			<input v-model="searchForm.chaoshiName" type="text" placeholder="超市管理员名称"></input>
		</view>
		<view class="action">
			<button @tap="search"
	...
	<view class="cu-list menu-avatar" v-for="(item,index) in list" v-bind:key="index">
		<view class="cu-item" style="height: 310rpx;">
			<view class="content" style="left: 20rpx;">
				<view class="text-grey">超市管理员名称：{{item.chaoshiName}}</view>
	...
	<button
            ...
            v-if="isAuth('chaoshi','新增')" class="add-btn" @click="onAddTap()">新增</button>
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
				...
				let res = await this.$api.list(`chaoshi`, params);
				// 如果是第一页数据置空
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('chaoshi', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search() {
				...
				if (this.searchForm.chaoshiName) {
					searchForm['chaoshiName'] = this.searchForm.chaoshiName
				}
				let res = await this.$api.list(`chaoshi`, searchForm);
				// 如果是第一页数据置空
