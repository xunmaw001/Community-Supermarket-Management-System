...
<template>
	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
		...
		<view class="search-form round">
			...
			<input v-model="searchForm.dictionaryName" type="text" placeholder="字典表名称"></input>
		</view>
		<view class="action">
			<button @tap="search"
	...
	<view class="cu-list menu-avatar" v-for="(item,index) in list" v-bind:key="index">
		<view class="cu-item" style="height: 310rpx;">
			<view class="content" style="left: 20rpx;">
				<view class="text-grey">字典表名称：{{item.dictionaryName}}</view>
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
				...
				let res = await this.$api.list(`dictionary`, params);
				// 如果是第一页数据置空
			...
			async search() {
				...
				if (this.searchForm.dictionaryName) {
					searchForm['dictionaryName'] = this.searchForm.dictionaryName
				}
				let res = await this.$api.list(`dictionary`, searchForm);
				// 如果是第一页数据置空
