        ...
        <div v-if="showFlag">
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           ...
                           href="http://localhost:8080/shequchaosguanli/upload/shangpinMuBan.xls"
                        >批量导入商品信息数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('shangpin','导入导出')"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="shangpinJiliang"
                                   header-align="center"
                                   label="计量单位">
                        <template slot-scope="scope">
                            {{scope.row.shangpinJiliang}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('shangpin','修改')" type="primary" icon="el-icon-add-location" size="mini" @click="plusShangpinKucunNumber(scope.row.id,scope.row.shangpinKucunNumber,2)">增加库存</el-button>
                            <el-button v-if="isAuth('shangpin','修改')" type="primary" icon="el-icon-delete-location" size="mini" @click="plusShangpinKucunNumber(scope.row.id,scope.row.shangpinKucunNumber,1)">减少库存</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
        ...
        methods: {
            ...
            //增加库存
			plusShangpinKucunNumber(thisId,shangpinKucunNumber,types){
			//清空数据
				//库存数据设置为0
				this.kucunNumber = 0
				//id清空
				...
				//出入库类型
				this.shangpinchurukuTypes = null
				//操作类型清空
			...
			//数据不为空放值
				...
				//出入库类型
				this.shangpinchurukuTypes = types
				//原有数量
			...
			addShangpinKucunNumber(){
				...
				var type = null
				var maxNumber = 0
				if(_this.kucunNumberTypes == "+"){
					type = '增加'
					maxNumber = _this.shangpinKucunNumber + _this.kucunNumber
				}else{
					type = '减少'
					if(_this.shangpinKucunNumber < _this.kucunNumber || _this.kucunNumber <= 0){
						_this.$message.error('库存不足不能减少');
						return false;
					}else{
						maxNumber = _this.shangpinKucunNumber - _this.kucunNumber
					}
				...
				let date = {
					"id":_this.kucunNumberId,
					"shangpinKucunNumber":maxNumber
				}
				_this.$confirm(`确定要`+type+`物品数量`+_this.kucunNumber+`个?`, "提示", {
				    confirmButtonText: "确定",
				    cancelButtonText: "取消",
				    type: "warning"
				}).then(() => {
				    _this.$http({
				        ...
				        if (data && data.code === 0) {
							...
							let params = {
								shangpinId: _this.kucunNumberId,
								shangpinchurukuNumber: _this.kucunNumber,
								shangpinchurukuTypes: _this.shangpinchurukuTypes,
							}
							_this.$http({
							    url: "shangpinchuruku/save",
							    method: "post",
							    data: params
							}).then(({ data }) => {
							});
				...
				_this.shangpinKucunNumberVisible = false;
			},			//上下架
